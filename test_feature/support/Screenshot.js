...
class Screenshot {
    ...
    async create(fileName) {
        try {
            ...
            const curentUrl = await this.world.driver.getCurrentUrl();

            // Attaching screenshot username password and url to report
            await this.world.attach(`UserName : ${this.world.userName}`);
            await this.world.attach(`Password : ${this.world.password}`);
            await this.world.attach(`Failed in url : ${curentUrl}`);
